...
# %% auto 0
__all__ = ['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'openai_model', 'anthropic_model', 'model_provider', 'GAME_STATE_FILE',
           'MAX_HOPS', 'python_repl', 'tools', 'get_llm', 'cleanup_json', 'call_llm', 'get_game_states',
...
def get_llm():
    if model_provider == "openai":
        ...
        return ChatAnthropic(api_key=ANTHROPIC_API_KEY, model=anthropic_model)
    else:
        raise Exception(f"Unknown model provider: {model_provider}")

# %% carmen.ipynb 4
def cleanup_json(json_str: str):
    json_str = json_str.replace("```json", "")
    return json_str.replace("```", "")

# %% carmen.ipynb 5
def call_llm(prompt: str):
    return cleanup_json(get_llm().invoke(prompt).content)
...
def generate_new_game():
    ...
    return call_llm(new_game_prompt)
